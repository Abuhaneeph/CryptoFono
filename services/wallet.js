...
if (usdcBalance < parseFloat(amount) + 0.1) {
  // Get insufficient balance message template
  ...
  try {
    ...
    if (result.success) {
      ...
      if (customerResult.length > 0) {
        ...
        // Update the transaction record with correct fields that exist in the table
        
        await db.query(
          'UPDATE transactions SET recipient_id = ?, transaction_type = ? WHERE tx_hash = ?',
          [merchantId, 'merchant_payment', result.txHash]
        );
